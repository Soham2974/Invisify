...
export default function ScanPageClient() {
  ...
  const handleSubmit = async (data: FormValues) => {
    ...
    if ('error' in res) {
      ...
      // Sanitize the 'findings' string to remove massive arrays (like entropyMap) before storing in localStorage
      // Browser localStorage has a strict ~5MB limit. Without this, scanning images rapidly exceeds the quota and wipes the DB!
      try {
        const parsedFindings = JSON.parse(res.findings);
        if (parsedFindings?.image?.stegoveritas_analysis?.entropyMap) {
           parsedFindings.image.stegoveritas_analysis.entropyMap = ["[Data omitted for storage optimization]"];
        }
        res.findings = JSON.stringify(parsedFindings);
      } catch (e) {
              ...
              <div className="flex flex-col xl:flex-row min-h-[500px]">
                ...
                <div className="xl:w-2/5 p-8 bg-black/20 flex flex-col">
                  <div className="flex-1">
                    ...
                    {result && !isLoading && (() => {
                      // Parse findings from JSON string (actions.ts serializes it)
                      let parsedFindings: any = {};
                      try {
                        parsedFindings = typeof result.findings === 'string' ? JSON.parse(result.findings) : result.findings || {};
                      } catch (e) { parsedFindings = {}; }

                      const ensembleConfidence = parsedFindings.ensemble_confidence || 0.5;
                      ...
                      <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="space-y-4">
                          ...
                          <div className={cn(
                            ...
                            <div className="ml-auto text-right">
                              <div className="text-[10px] font-mono opacity-50 uppercase mb-0.5">Threat Score</div>
                              <div className="text-2xl font-black tracking-tighter">{result.score}%</div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                            <div className="text-[10px] font-mono text-neutral-500 uppercase mb-1">Method</div>
                            <div className="text-sm font-bold uppercase">{hasAI ? 'AI + STAT' : 'DETERMINISTIC'}</div>
                          </div>
                          <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                            <div className="text-[10px] font-mono text-neutral-500 uppercase mb-1">Confidence</div>
                            <div className="text-sm font-bold">{(ensembleConfidence * 100).toFixed(1)}%</div>
                          </div>
                          <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                            <div className="text-[10px] font-mono text-neutral-500 uppercase mb-1">Detectors</div>
                            <div className="text-sm font-bold">{detectorsTriggered}/{detectorsTotal}</div>
                          </div>
                        </div>
                      </div>
                      );
